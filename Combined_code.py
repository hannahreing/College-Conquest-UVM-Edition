...
def try_swap(r1, c1, r2, c2):
    ...
    if matchfinder(gameboard):
        ...
        popup = tk.Toplevel(gameboard.window)
        popup.title("Match found")
        popup.attributes('-topmost', True)
        popup.transient(gameboard.window)
        tk.Label(popup, text=adjectives, font=(
            "Arial", 16, "bold")).pack(pady=20)
        gameboard.window.after(2000, popup.destroy)
    else:
